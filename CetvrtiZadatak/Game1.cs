...
namespace CetvrtiZadatak
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            // Paddles implementation
            var touchState = Keyboard.GetState();
            if (touchState.IsKeyDown(Keys.Left))
            {
                PaddleBottom.X = PaddleBottom.X - (PaddleBottom.Speed * (float)gameTime.ElapsedGameTime.TotalMilliseconds);
            }
            ...
            {
                PaddleTop.X = PaddleTop.X - (PaddleTop.Speed * (float)gameTime.ElapsedGameTime.TotalMilliseconds);
            }
            if (touchState.IsKeyDown(Keys.D))
            {
                PaddleTop.X = PaddleTop.X + (PaddleTop.Speed * (float)gameTime.ElapsedGameTime.TotalMilliseconds);
            }
            PaddleBottom.X = MathHelper.Clamp(PaddleBottom.X, graphics.GraphicsDevice.Viewport.Bounds.Left, 
                graphics.GraphicsDevice.Viewport.Bounds.Right - PaddleBottom.Width);
            PaddleTop.X = MathHelper.Clamp(PaddleTop.X, graphics.GraphicsDevice.Viewport.Bounds.Left, 
                graphics.GraphicsDevice.Viewport.Bounds.Right - PaddleTop.Width);
            ...
            {
                Ball.X = graphics.GraphicsDevice.Viewport.Bounds.Center.ToVector2().X;
                Ball.Y = graphics.GraphicsDevice.Viewport.Bounds.Center.ToVector2().Y;
